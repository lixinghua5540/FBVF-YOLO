...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            out = model(im) if compute_loss else (model(im, augment=augment), None)
            if task == 'val':
                preds, train_out= out #如果是train阶段。改成: preds, train_out, mask = out
            else:
                preds, train_out, mask = out
...
def parse_opt():
    ...
    parser.add_argument("--name", default="RS-STOD", help="save to project/name")
